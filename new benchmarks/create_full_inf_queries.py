...
def compute_fi_answers(query_structure, query, test_only_ent_out, train_ent_out, train_answer_set, test_answer_set,
                       rel_per_query, anch_per_query):
    # check all possible combinations (train+test) from which an answer can be retrieved, and only retain the answers that can be retrieved by test links only
    # 1p
    if query_structure == ['e', ['r']]:
        entity = query[0]
        rel1 = query[1][0]
        if rel1 in rel_per_query:
            rel_per_query[rel1] += len(test_answer_set)
        else:
            rel_per_query[rel1] = len(test_answer_set)
        if entity in anch_per_query:
            anch_per_query[entity] += len(test_answer_set)
        else:
            anch_per_query[entity] = len(test_answer_set)
        return 1, test_answer_set, rel_per_query, anch_per_query, [rel1], [entity]
    # 2p
    if query_structure == ['e', ['r', 'r']]:
        ...
        if len(reachable_answers_2p) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_2p)
                else:
                    rel_per_query[rel] = len(reachable_answers_2p)
        ...
        return len(reachable_answers_2p),reachable_answers_2p, rel_per_query, anch_per_query, [rel1, rel2], [entity]
    # 3p
    if query_structure == ['e', ['r', 'r', 'r']]:
        ...
        if len(reachable_answers_3p) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_3p)
                else:
                    rel_per_query[rel] = len(reachable_answers_3p)
        ...
        return len(reachable_answers_3p),reachable_answers_3p, rel_per_query, anch_per_query, [rel1, rel2, rel3], [entity]
    ...
    if query_structure == ['e', ['r', 'r', 'r', 'r']]:
        ...
        return len(reachable_answers_4p),reachable_answers_4p, rel_per_query, anch_per_query, [rel1, rel2, rel3, rel4], [entity]
    ...
    if query_structure == [['e', ['r']], ['e', ['r']]]:
        ...
        if len(reachable_answers_2i) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_2i)
                else:
                    rel_per_query[rel] = len(reachable_answers_2i)
            set_ent = ({entity1, entity2})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_2i)
                else:
                    anch_per_query[ent] = len(reachable_answers_2i)
        return len(reachable_answers_2i),reachable_answers_2i, rel_per_query, anch_per_query, [rel1, rel2], [entity1, entity2]
    # 3i
    if query_structure == [['e', ['r']], ['e', ['r']], ['e', ['r']]]:
        ...
        if len(reachable_answers_3i) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_3i)
                else:
                    rel_per_query[rel] = len(reachable_answers_3i)
            set_ent = ({entity1, entity2, entity3})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_3i)
                else:
                    anch_per_query[ent] = len(reachable_answers_3i)

        return len(reachable_answers_3i),reachable_answers_3i, rel_per_query, anch_per_query, [rel1, rel2, rel3], [entity1, entity2, entity3]
    ...
    if query_structure == [['e', ['r']], ['e', ['r']], ['e', ['r']], ['e', ['r']]]:
        ...
        return len(reachable_answers_4i),reachable_answers_4i, rel_per_query, anch_per_query, [rel1, rel2, rel3, rel4], [entity1, entity2, entity3, entity4]
    ...
    if query_structure == [['e', ['r', 'r']], ['e', ['r']]]:
        ...
        if len(reachable_answers_pi) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_pi)
                else:
                    rel_per_query[rel] = len(reachable_answers_pi)
            set_ent = ({entity1, entity2})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_pi)
                else:
                    anch_per_query[ent] = len(reachable_answers_pi)
        return len(reachable_answers_pi),reachable_answers_pi, rel_per_query, anch_per_query, [rel1, rel2, rel3], [entity1, entity2]
    # ip
    if query_structure == [[['e', ['r']], ['e', ['r']]], ['r']]:
        ...
        if len(reachable_answers_ip) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_ip)
                else:
                    rel_per_query[rel] = len(reachable_answers_ip)
            set_ent = ({entity1, entity2})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_ip)
                else:
                    anch_per_query[ent] = len(reachable_answers_ip)

        return len(reachable_answers_ip),reachable_answers_ip, rel_per_query, anch_per_query, [rel1, rel2, rel3], [entity1, entity2]
    # up
    if query_structure == [[['e', ['r']], ['e', ['r']], ['u']], ['r']]:
        ...
        for ele in answers_10:
            answers_100.update(train_ent_out[ele][rel3])
        reachable_answers_0p = (answers_010 | answers_100) - train_answer_set  
        # this set should already be empty but we enforce that if there are answers in here those are filtered out next
        ...
        if len(reachable_answers_up) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_up)
                else:
                    rel_per_query[rel] = len(reachable_answers_up)
            set_ent = ({entity1, entity2})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_up)
                else:
                    anch_per_query[ent] = len(reachable_answers_up)
        return len(reachable_answers_up),reachable_answers_up, rel_per_query, anch_per_query, [rel1, rel2, rel3], [entity1, entity2]
    ...
    if query_structure == [['e', ['r']], ['e', ['r']], ['u']]:
        ...
        if len(reachable_answers_2u) > 0:
            ...
            for rel in set_rel:
                if rel in rel_per_query:
                    rel_per_query[rel] += len(reachable_answers_2u)
                else:
                    rel_per_query[rel] = len(reachable_answers_2u)
            set_ent = ({entity1, entity2})
            for ent in set_ent:
                if ent in anch_per_query:
                    anch_per_query[ent] += len(reachable_answers_2u)
                else:
                    anch_per_query[ent] = len(reachable_answers_2u)
        return len(reachable_answers_2u),reachable_answers_2u, rel_per_query, anch_per_query, [rel1, rel2], [entity1, entity2]
...
def ground_queries(dataset, query_structures, ent_in, ent_out, train_ent_in, train_ent_out, test_only_ent_in,
    ...
    for idx, query_structure in enumerate(query_structures):
        ...
        if query_name == "1p" and mode != "train" and dataset== "ICEWS18+H": # test 1p are kept to the same dimensions are for the other benchmarks, where the given 1p is re-used
            gen_num = 5000
        else:
        ...
        while num_sampled < gen_num:
            ...
            train_answer_set = achieve_answer(query, train_ent_in, train_ent_out)  # answers retrievable by the training set only
            test_answer_set = answer_set - train_answer_set # answers retrievable by the (training + test set) - train_answer_set
            ...
            n_sampled_step, pred_answers, rel_per_query_temp, anch_per_query_temp, rels, anchs = compute_fi_answers(
                query_structure,
                query,
                test_only_ent_out,
                train_ent_out,
            ...
            if num_sampled > gen_num / 8: # enforcing that we do not have more than 20% of one rel name nor anchor for each query type
                top_k_rel_keys, top_k_rel_values = top_k_dict_values_sorting(rel_per_query_temp, 1)
                perc_top_rel = (top_k_rel_values[0] * 100) / (tot_qa_pairs + len(pred_answers))

                top_k_anch_keys, top_k_anch_values = top_k_dict_values_sorting(anch_per_query_temp, 1)
                perc_top_anch = (top_k_anch_values[0] * 100) / (tot_qa_pairs + len(pred_answers))
            ...
            train_answers[list2tuple(query)] = answer_set - pred_answers  # train answer set
            hard_answers[list2tuple(query)] = pred_answers  # hard answer set
            num_sampled += n_sampled_step
            if query_name == "1p":
                tot_qa_pairs += len(pred_answers) #count queries
            else:
                tot_qa_pairs = num_sampled #count answers
    ...
    name_to_save = '%s-' % (mode)
    with open('./data/%s/%s-%s-0-full-inf-queries.pkl' % (dataset, name_to_save,seed), 'wb') as f:
        pickle.dump(queries, f)
    with open('./data/%s/%s-%s-0-full-inf-easy-answers.pkl' % (dataset, name_to_save,seed), 'wb') as f:
        pickle.dump(train_answers, f)
    with open('./data/%s/%s-%s-0-full-inf-hard-answers.pkl' % (dataset, name_to_save,seed), 'wb') as f:
        pickle.dump(hard_answers, f)
...
@click.command()
@click.option('--dataset', default="FB15k-237+H")
