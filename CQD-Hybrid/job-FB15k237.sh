...
for param_task in "${params_tasks[@]}"; do
    if [ "$param_task" = "2p" ]; then
      ...
      cqd_k="512"
      cqd_t_norm="min"
      params_subtasks=("None" "2u")
    elif [ "$param_task" = "up" ]; then
      cqd_k="512"
      cqd_t_norm="min"
      params_subtasks=("None" "ov" "1p" "2u" "up")
    fi
    for params_subtask in "${params_subtasks[@]}"; do
        python main.py --do_test --data_path ../old_benchmarks/FB15k-237-betae -n 1 -b 1000 -d 1000 --cpu_num 0 --geo cqd --print_on_screen --test_batch_size 1 --checkpoint_path models/fb15k-237-betae --cqd discrete --cqd-t-norm "$cqd_t_norm" --cqd-k $cqd_k --tasks "$param_task" --subtask "$params_subtask" --cqd-max-norm 0.9 --cqd-normalize --cqd-max-k 512
    done
done